...
public class ClasesView extends Composite<VerticalLayout> {
    ...
    public ClasesView(ClaseController claseController, ClaseRepository claseRepository, ParticipacionController participacionesController) {
        ...
        HorizontalLayout searchButtonLayout = new HorizontalLayout(searchButton, clearSearchButton, deleteSelectedButton);
        ...
        deleteSelectedButton.addClickListener(e -> deleteSelectedClases());
    ...
    private void deleteSelectedClases() {
        Set<Clase> seleccionadas = grid.getSelectedItems();

        if (seleccionadas == null || seleccionadas.isEmpty()) {
            Notification.show("No hay clases seleccionadas.", 3000, Notification.Position.TOP_CENTER)
                .addThemeVariants(NotificationVariant.LUMO_ERROR);
            return;
        }

        // Verificar si alguna clase tiene relaciones
        List<Clase> noEliminables = seleccionadas.stream()
            .filter(this::claseTieneRelaciones)
            .toList();
        ...
        if (!noEliminables.isEmpty()) {
            ...
            String clasesConRelaciones = noEliminables.stream()
                .map(c -> c.getCodigoClase())
                .collect(Collectors.joining(", "));

            aviso.add(new Text("Las siguientes clases tienen relaciones con otras tablas y no pueden eliminarse:\n"
                + clasesConRelaciones));
            ...
            confirmDialog.add(new Text("¿Estás seguro de eliminar " + seleccionadas.size() + " clase(s)?"));

            Button eliminar = new Button("Eliminar", evt -> {
                try {
                    seleccionadas.forEach(clase -> claseController.delete(clase.getId()));
                    confirmDialog.close();
                    refreshGrid(); // Actualizar tabla
                    Notification.show("Clases eliminadas correctamente", 3000, Notification.Position.TOP_CENTER)
                        .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
                } catch (Exception e) {
                    Notification.show("Error al eliminar las clases: " + e.getMessage(), 3000, Notification.Position.TOP_CENTER)
                        .addThemeVariants(NotificationVariant.LUMO_ERROR);
                }
            });
            ...
            confirmDialog.getFooter().add(new HorizontalLayout(eliminar, cancelar));
            confirmDialog.open();
        }
    }
   ...
   // Método para buscar clases según los criterios ingresados
    ...
    private void refreshGrid() {
        ...
        grid.deselectAll();
        deleteSelectedButton.setEnabled(false); // Deshabilitar el botón de eliminar si no hay selección
